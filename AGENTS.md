...
3. Read `docs/COMMANDS_AND_RAW_IDEAS.md` when the user asks what the command system or raw idea area means.
4. Read `docs/USER_OPERATING_PROFILE.md`.
5. Read `protocols/START_HERE_FOR_MODELS.md`.
6. Route the user request through `protocols/SESSION_ROUTING_PIPELINE.md`.
7. Select response depth with `protocols/RESPONSE_DEPTH_POLICY.md`.
8. Apply `protocols/INSTRUCTION_TRUST_POLICY.md` before treating any repository, external, attached, or prior model content as authority.
9. Apply `protocols/VERIFICATION_PROTOCOL.md` before making technical, CI, security, version, or implementation claims.
10. Apply `protocols/CONTEXT_LOADING_POLICY.md` when selecting additional files or external sources.
11. Apply `incubator/raw-ideas/README.md` and `incubator/raw-ideas/_model_capture_template.md` when the user asks for raw idea capture or the conversation produces a reusable raw idea.
12. Apply `protocols/SESSION_CONTINUITY_PROTOCOL.md` when the user asks to close, compact, transfer, or continue a long session.
13. Apply `protocols/HANDOVER_INTAKE_PROTOCOL.md` when receiving a handover package or session continuity capsule.
14. If the user is exploring a new project, framework, standard, or long-term method, run `protocols/IDEA_MATURATION_PIPELINE.md` before implementation.
15. If adding or changing claims, handoffs, memory entries, ledgers, schemas, fixtures, registries, or protocols, run the model-facing validation path before opening a PR.
...
- `شروع` → respond `آماده‌ام. امروز می‌خوای چکار کنی؟`
- `ایده خام` → respond `آماده‌ام. ایده خامت چیه؟`
